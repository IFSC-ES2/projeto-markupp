...
INTEGRATION_PKG=./internal/api
DOCKER_COMPOSE=docker compose

.PHONY: all test-db-config compose-config compose-env docker-up docker-test docker-down run run-air
...
docker-test:
	$(DOCKER_COMPOSE) -f $(COMPOSE_FILE) exec backend sh -c "cd $(BACKEND_WORKDIR) && go test ./..."
