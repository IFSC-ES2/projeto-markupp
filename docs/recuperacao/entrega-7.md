...
(≥ 6): **5,6 / 5,4 / 4,4 / 5,0**. O principal motivo apontado foi que o
incremento da Sprint 3 **não havia sido consolidado na branch principal** nem
empacotado em uma release/tag do marco — havia trabalho em PRs sobre `dev`, mas
não integrado em `main`.

- Branch de recuperação: `rec-7` (regra 3).
- Forma de entrega: pull request `rec-7 → main` (regra 4).
...
- **(b) Correção:** a Sprint 3 foi integrada à branch principal e empacotada no
  ...
  (PR #74) e a reengenharia de nomenclatura `backend → markupp` /
  `backendUrl → serverUrl` (PR #73).
...
- **(b) Correção:** consolidação dos ADRs do marco, incluindo a decisão de
  licença AGPL (ADR-0008), o rename `backend → markupp` (ADR-0009) e o
...
- **(b) Correção:** as fichas foram separadas em arquivos numerados, cada uma com
  ...
  ciclos.
- **(c) Evidência:** `docs/metricas/01-cobertura-de-testes.md` …
...
- **Pendência conhecida:** as fichas trazem os gráficos por sprint, mas ainda
  ...
  tendência/comparação Sprint 2 → Sprint 3.
